...
// Stateful Component
export default function Lottery({lottery, removeLotteryNumbers, handleChange, draw, reset}) {
    let table = ""
    if (lottery.numbers.length > 0) {
        table = <table className="table table-bordered table-hover table-striped">
            ...
            <tbody>{
                lottery.numbers.map((lotteryNumbers, index) =>
                    <tr key={index}>
                        <td>{index + 1}</td>
                        {
                        ...
                        <td>
                            <button onClick={() => removeLotteryNumbers(index)}
